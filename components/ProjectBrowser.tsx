...
export default function ProjectBrowser({ projects }: { projects: Project[] }) {
  ...
  const spaces = useMemo(() => Array.from(new Set(projects.flatMap((p) => p.spaces))), [projects]);
  const filtered = projects.filter((p) => {
    const q = query.trim().toLowerCase();
    const matchesQuery = !q || [p.title, p.city, p.district, p.apartment, p.squareNumber].join(" ").toLowerCase().includes(q);
  ...
  return <>
    <div className="project-controls">
      ...
      <div className="filter-row">
        ...
        <label className="search-field"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="아파트명(지역명) 또는 스퀘어 넘버를 검색해보세요." /></label>
      </div>
    </div>
    <div className="portfolio-grid">
      {filtered.map((project) => <Link href={`/project/${project.slug}`} className="portfolio-item" key={project.slug}>
        <div className="portfolio-image placeholder-image"><div className="placeholder-inner"><span>{project.squareNumber}</span><small>PROJECT IMAGE</small></div><div className="project-hover">{project.apartment}</div></div>
        <div className="portfolio-caption"><strong>{project.apartment}</strong><span>{project.city} {project.district} · {project.area}평</span></div>
      </Link>)}
